...
function moviabrir() {//Define cuanto se va abrir la puerta(angulo)
  let af = 45;
  if(movimiento<=1){
	Rota3D.initRotate( obj.w[139], obj.w[140], af*Math.PI/180);	//Puntos del lado que debe de girar(Puerta)
  ...
  cv.paint();	
  movimiento++;
}else{alert('El microondas no se puede abrir más');}
}
...
document.getElementById('pza1Izq').addEventListener('click', moviabrir, false);
document.getElementById('pza1Der').addEventListener('click', movicerrar, false);
